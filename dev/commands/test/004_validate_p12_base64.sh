echo ""
echo ""
echo "***** 004_validate_p12_base64 *****" 
echo "-----------------------------------------------------------------------"
echo "Installing SecretProviderClass - p12 - base64" 
echo "-----------------------------------------------------------------------"
...
spec:
  provider: appviewx
  parameters:
    objectFormat: p12    # pem, pfx, p12, jks
...
kubectl exec -it webapp -- cat /etc/ssl/tls.p12 | base64 --decode > ./tls.p12
kubectl exec -it webapp -- cat /etc/ssl/password > ./password
openssl pkcs12 -in ./tls.p12   -passin pass:$(cat ./password) --passout pass:$(cat ./password) -nocerts -nodes | sed -ne '/-BEGIN PRIVATE KEY-/,/-END PRIVATE KEY-/p' > ./tls.key
openssl pkcs12 -in ./tls.p12   -passin pass:$(cat ./password) --passout pass:$(cat ./password) -clcerts -nokeys | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ./tls.crt
openssl pkcs12 -in ./tls.p12   -passin pass:$(cat ./password) --passout pass:$(cat ./password) -cacerts -nokeys  -chain | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ./ca.crt
