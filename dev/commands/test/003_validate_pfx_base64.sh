echo ""
echo ""
echo "***** 003_validate_pfx_base64 *****" 
echo "-----------------------------------------------------------------------"
echo "Installing SecretProviderClass - pfx - base64" 
echo "-----------------------------------------------------------------------"
...
spec:
  provider: appviewx
  parameters:
    objectFormat: pfx    # pem, pfx, p12, jks
...
kubectl exec -it webapp -- cat /etc/ssl/tls.pfx | base64 --decode > ./tls.pfx
kubectl exec -it webapp -- cat /etc/ssl/password > ./password
openssl pkcs12 -in ./tls.pfx   -passin pass:$(cat ./password) --passout pass:$(cat ./password) -nocerts -nodes | sed -ne '/-BEGIN PRIVATE KEY-/,/-END PRIVATE KEY-/p' > ./tls.key
openssl pkcs12 -in ./tls.pfx   -passin pass:$(cat ./password) --passout pass:$(cat ./password) -clcerts -nokeys | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ./tls.crt
openssl pkcs12 -in ./tls.pfx   -passin pass:$(cat ./password) --passout pass:$(cat ./password) -cacerts -nokeys  -chain | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > ./ca.crt
