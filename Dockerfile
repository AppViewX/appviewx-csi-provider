...
# -----------------------------------
FROM repo.appviewx.in/hashicorp/golang:1.17.6 AS devbuild
ENV CGO_ENABLED=0
# Leave the GOPATH
WORKDIR /build
COPY . ./
...
# -----------------------------------
FROM repo.appviewx.in/hashicorp/alpine:3.15.0 AS dev
COPY --from=devbuild /build/appviewx-csi-provider /bin/
ENTRYPOINT [ "/bin/appviewx-csi-provider" ]

# Default release image.
# -----------------------------------
FROM repo.appviewx.in/hashicorp/alpine:3.15.0 AS default
